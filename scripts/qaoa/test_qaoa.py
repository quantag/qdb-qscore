...
def solve_with_csv_file(csv_path: str):
    ...
    # Call API
    resp = requests.post(f"{BASE_URL}/solve", json=payload, verify=False)  # verify=False if self-signed cert
    try:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test_api20_file.py <portfolio.csv> [health]")
        sys.exit(1)
